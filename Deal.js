function Deal (dealWho : String) {
	...
	{
		case 1: soundDeal1.Play (); break;
		case 2: soundDeal2.Play (); break;
		case 3: soundDeal3.Play (); break;
	}
	...
	if (dealWho == "dealer")
	{
		...
		var tempOffsetDealer : float = 0.2343085;

		if (PlayerPrefs.GetInt("CardTheme") == 8)
			tempOffsetDealer = 0.3;

		var clone = Instantiate (dealerHand[m], Vector3 (0 + m * tempOffsetDealer, 2.92, -m * 0.01), Quaternion.Euler (0,0,0));
		clone.GetComponent.<Renderer>().sortingOrder = zOrder + chipOrder;
		zOrder++;
		
		if (m == 1)
		{
			originalSprite = clone.GetComponent(SpriteRenderer).sprite;
			clone.GetComponent.<Renderer>().material = cardMatClosed;
			clone.GetComponent(SpriteRenderer).sprite = closedCard;
			clone.GetComponent(Card).closed = true;
			clone.GetComponent(Card).onceClosed = true;
	...
	else if (dealWho == "player")
	{
		...
		var tempOffset : float = 0.2343085;
		...
		if (split)
		{
			var cardSplit = Instantiate (playerHand[j], Vector3 (-1.35 + j * tempOffset, -2.52, -j * 0.01), Quaternion.Euler (0,0,0)); // + j * 0.1843085
			cardSplit.GetComponent.<Renderer>().sortingOrder = zOrder + chipOrder;
		...
		else if (doubledDown)
		{
			...
			cardDoubledown.GetComponent.<Renderer>().sortingOrder = zOrder + chipOrder;
		...
		else
		{
			if (j == 0)
			{
				playerCard1 = Instantiate (playerHand[j], Vector3 (0.1 + j * tempOffset, -2.62, -j * 0.01), Quaternion.Euler(0,0,0)); //  + j * 0.1843085 // -2.984436
				playerCard1.GetComponent.<Renderer>().sortingOrder = zOrder + chipOrder;
			...
			{
				playerCard2 = Instantiate (playerHand[j], Vector3 (0.1 + j * tempOffset, -2.62, -j * 0.01), Quaternion.Euler(0,0,0)); //  + j * 0.1843085 // -2.984436
				playerCard2.GetComponent.<Renderer>().sortingOrder = zOrder + chipOrder;
			...
			{
				var cardNormal = Instantiate (playerHand[j], Vector3 (0.1 + j * tempOffset, -2.62, -j * 0.01), Quaternion.Euler(0,0,0)); //  + j * 0.1843085 // -2.984436
				cardNormal.GetComponent.<Renderer>().sortingOrder = zOrder + chipOrder;
	...
	else if (dealWho == "split")
	{
		...
		var tempOffsetSplit : float = 0.2343085;

		if (PlayerPrefs.GetInt("CardTheme") == 8)
			tempOffsetSplit = 0.3;

		var splitCard = Instantiate (splitHand[y], Vector3 (1.65 + y * tempOffsetSplit, -2.52, -y * 0.01), Quaternion.Euler(0,0,0)); //  + y * 0.1843085
        splitCard.GetComponent(Card).cardOrder = y;
		splitCard.GetComponent.<Renderer>().sortingOrder = zOrder + chipOrder;
		zOrder++;
